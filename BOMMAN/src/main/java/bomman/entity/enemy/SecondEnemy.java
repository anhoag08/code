...
public class SecondEnemy extends CommonEntity {
    ...
    public void render(GraphicsContext gc, double t) {
        ...
        if (getAlive() == 1) {
            if (frame == 0) count++;
            if (count == 1) {
                this.setImg(Sprite.newEnemy2_up2.getFxImage());
            }
            if (count == 20) {
            ...
            if (this.getDirect() == DIRECTION.LEFT) {
                if (frame == 0) this.setImg(Sprite.newEnemy2_left1.getFxImage());
                if (frame == 1) this.setImg(Sprite.newEnemy2_left2.getFxImage());
                if (frame == 2) this.setImg(Sprite.newEnemy2_left3.getFxImage());
            }
            if (this.getDirect() == DIRECTION.RIGHT) {
                if (frame == 0) this.setImg(Sprite.newEnemy2_right1.getFxImage());
                if (frame == 1) this.setImg(Sprite.newEnemy2_right2.getFxImage());
                if (frame == 2) this.setImg(Sprite.newEnemy2_right3.getFxImage());
            }
            if (this.getDirect() == DIRECTION.UP) {
                if (frame == 0) this.setImg(Sprite.newEnemy2_up1.getFxImage());
                if (frame == 1) this.setImg(Sprite.newEnemy2_up2.getFxImage());
                if (frame == 2) this.setImg(Sprite.newEnemy2_up3.getFxImage());
            }
